...
/**
 * Performance test of inserting data into Cassandra users CF.
 * 
 * To run in Spark standalone mode 
 * 
 * /path/to/spark-submit 
 *   --class com.acme.perf.UsersUpsertPerf 
 *   --master local[1] 
 ...
 */
object UsersUpsertPerf extends Perf {
  ...
  def main(args: Array[String]) {
    ...
    val total = sc.textFile(appConfig.inputFolder)
      .map { 
        line => {
          ...
          track foreach {
            obj => {
              UsersOps.upsert(obj).onComplete {
                case Success(value) => onSuccess(obj)
    ...
    println(s" = total successful upserts $nrSuccess, failed upserts $nrFailure")
    
    sc.stop()
    System.exit(0)
  }

  def onSuccess(user: Track) = {
    Lock.synchronized {
